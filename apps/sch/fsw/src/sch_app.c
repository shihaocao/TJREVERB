/*
** $Id: sch_app.c 1.14 2012/06/26 13:56:10EDT lwalling Exp  $
**
...
********************************************************************/
#if 0
extern unsigned long     uSecondsUp;
extern unsigned long     uSubSeconds;
#endif
...
#if 0
printf("TIME Stats: \n");
printf("vGetRuntimeCounterValue = %d\n",vGetRuntimeCounterValue());
printf("uSecondsUp = %d\n",uSecondsUp);
printf("uSubSeconds = %d\n",uSubSeconds);
#endif
...
#if 0
extern uint32 CFE_PSP_ST1Count;
if ( SCH_AppData.NextSlotNumber == 9 )
{
   printf("Time = %d\n",vGetRuntimeCounterValue());
   printf("C:%d N:%d\n",CurrentSlot,SCH_AppData.NextSlotNumber);
   printf("ST1 int count = %d\n", CFE_PSP_ST1Count);
}
#endif
...
#if 0
            printf("   SCH-->CurrentSlot = %d\n",CurrentSlot);
            printf("   SCH-->NextSlotNumber = %d\n",SCH_AppData.NextSlotNumber);
            printf("   SCH-->ProcessCount = %d\n",ProcessCount);
            if ( ProcessCount > SCH_AppData.WorstCaseSlotsPerMinorFrame )
            {
               printf("  SCH-->ProcessCount:%d > WorstCaseSlotsPerMinorFrame:%d\n", 
                       ProcessCount, SCH_AppData.WorstCaseSlotsPerMinorFrame);
            }
            if ( SCH_AppData.SyncToMET == SCH_NOT_SYNCHRONIZED )
            {
               printf("  SCH-->SyncToMET is NOT_SYNC\n");
            }
#endif
        }
    }
