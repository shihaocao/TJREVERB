...
void TO_StartSending( TO_OUTPUT_ENABLE_PKT_t * pCmd )
{
    CFE_EVS_SendEvent(TO_TLMOUTENA_INF_EID,CFE_EVS_INFORMATION,"TO telemetry output enabled");
...
{
   OS_printf("TO_LAB: Opened downlink connection\n\r");
    ...
    do
    {
       ...
       if ( (CFE_SB_status == CFE_SUCCESS) && (suppress_sendto == FALSE) )
       {
          ...
          if(downlink_on == TRUE)
          {
             ...
             OS_printf("TO_LAB: Send Packet to Downlink: 0x%04X\n",MsgId);
