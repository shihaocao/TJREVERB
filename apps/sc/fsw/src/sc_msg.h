/************************************************************************
** File:
**   $Id: sc_msg.h 1.12 2011/09/23 14:26:16EDT lwalling Exp  $
**
** Purpose: 
...
/** 
**  \brief RTS Unbundle Command
**  For command details see #SC_UNBUNDLE_RTS_CC
*/
typedef struct {
    
    uint8               CmdHeader[CFE_SB_CMD_HDR_SIZE];
    uint8               BundlePath[OS_MAX_PATH_LEN]; /**< \brief The path and filename of the RTS bundle */
    uint8               RtsPath[OS_MAX_PATH_LEN];    /**< \brief The destination path for the RTSs to unbundle */
    uint16              RtsSize;                     /**< \brief The size of the RTS files in the bundle ( must all be the same size )  */
    uint16              StartingRtsNum;              /**< \brief The number of the first RTS in the bundle ( must be contiguous ) */
}  SC_UnbundleRtsCmd_t;
