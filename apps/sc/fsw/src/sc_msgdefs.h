/************************************************************************
** File:
**   $Id: sc_msgdefs.h 1.8 2011/09/23 14:26:40EDT lwalling Exp  $
**
** Purpose: 
...
/** \sccmd UNBUNDLE a file of RTS
**  
**  \par Description
**       This command will unbundle a group of RTSs from a binary "bundle" in the nonvolatile
**       memory into individual RTS files in a RAM disk. 
**
**  \sccmdmnemonic \SC_UNBUNDLERTS
**
**  \par Command Structure
**       #SC_UnbundleRtsCmd_t
**
...
**       This command may fail for the following reason(s):
**       - Command packet length not as expected
**       - Invalid RTS Size, RtsSize must match SC_RTS_BUFF_SIZE * 2 + SC_RTS_FILE_OVERHEAD
**       - Invalid Starting RTS Number. The Starting RTS number must be at least SC_BUNDLE_FIRST_RTS 
**       - Invalid bundle path, The path to the bundle filename must be at least 3 characters 
**       - Invalid RTS path, The path to unbundle the RTS files must be at least 3 characters  
**       - Failure from unbundle function, The unbundle process may fail due to invalid paths or filenames, file errors, or the bundle file does not exist 
** 
**  \par Evidence of failure may be found in the following telemetry: 
**       - \b \c \SC_CMDEC - command error counter will increment
**       - The #SC_LEN_ERR_EID event will indicate invalid command packet length.
...
**       None
**
**  \sa #SC_UNBUNDLE_RTS_CC
*/
#define SC_UNBUNDLE_RTS_CC             17
