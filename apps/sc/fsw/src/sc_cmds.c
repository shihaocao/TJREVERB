 /*************************************************************************
 ** File:
 **   $Id: sc_cmds.c 1.17 2011/09/26 13:47:57EDT lwalling Exp  $
 **
 ** Purpose: 
 ...
 **
 **   $Log: sc_cmds.c  $ 
 **   Revision 1.17 2011/09/26 13:47:57EDT lwalling  
            ...
            (SC_OperData.AtsCtrlBlckAddr -> AtpState == SC_EXECUTING))
    {
        ...
        if (AbortATS == TRUE)
        {
            CFE_EVS_SendEvent(SC_ATS_ABT_ERR_EID, CFE_EVS_ERROR,
                             "Absolute Time Tagged Sequence %c Aborted",
                              TempAtsChar);
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/*                                                                 */
/* RTS Unbundle Command                                            */
/*                                                                 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
void SC_UnbundleCmd(CFE_SB_MsgPtr_t CmdPacket)
{
    SC_UnbundleRtsCmd_t *UnbundleCmd = (SC_UnbundleRtsCmd_t *) CmdPacket;
    int32                UnbundleStatus;

    if (SC_VerifyCmdLength(CmdPacket, sizeof(SC_UnbundleRtsCmd_t)))
    {
      
       if (( UnbundleCmd->RtsSize != (SC_RTS_BUFF_SIZE * 2) + SC_RTS_FILE_OVERHEAD )  ||
           ( UnbundleCmd->StartingRtsNum < SC_BUNDLE_FIRST_RTS || UnbundleCmd->StartingRtsNum > SC_NUMBER_OF_RTS ) ||
           ( strlen((char *)UnbundleCmd->BundlePath) < 3 ) ||
           ( strlen((char *)UnbundleCmd->RtsPath) < 3 ))
       {
           CFE_EVS_SendEvent(SC_UNBUNDLE_RTS_CMD_ERR_EID, CFE_EVS_ERROR,
                              "Unbundle RTS Cmd Inv Param: %s, Dest: %s, RTS Size: %d, RTS Num: %d",
                              UnbundleCmd->BundlePath,
                              UnbundleCmd->RtsPath,
                              UnbundleCmd->RtsSize,
                              UnbundleCmd->StartingRtsNum);
       ...
       {       
          ...
          UnbundleStatus = SC_UnbundleRTSs((char *)UnbundleCmd->BundlePath, 
                                           (char *)UnbundleCmd->RtsPath,
                                                   UnbundleCmd->RtsSize, 
                                                   UnbundleCmd->StartingRtsNum);
...
void SC_ProcessCommand (CFE_SB_MsgPtr_t CmdPacket)
{
    ...
    switch (CommandCode)
    {
        ...
        case SC_UNBUNDLE_RTS_CC:
            SC_UnbundleCmd(CmdPacket);
            break;
