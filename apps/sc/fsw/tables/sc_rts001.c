...
/************************************************************************
**
** $Id: sc_rts001.c 1.5 2010/04/22 13:30:42EDT lwalling Exp  $
**
...
uint16 RTS_Table001[SC_RTS_BUFF_SIZE] =
{
  /*  cmd time,  <---- cmd pkt primary header ---->  <----- cmd pkt 2nd header ---->   <-- opt data ---> */
     CMD1_TIME,  SC_CMD_MID, PKT_FLAGS, CMD1_LENGTH, ((SC_NOOP_CC << 8) | CMD1_XSUM),
     CMD2_TIME,  SC_CMD_MID, PKT_FLAGS, CMD2_LENGTH, ((SC_ENABLE_RTS_CC << 8) | CMD2_XSUM), CMD2_ARG,
     CMD3_TIME,  SC_CMD_MID, PKT_FLAGS, CMD3_LENGTH, ((SC_START_RTS_CC << 8) | CMD3_XSUM), CMD3_ARG
};
