/************************************************************************
**
** $Id: fm_utest_utils.c 1.4 2009/12/02 14:31:19EST lwalling Exp  $
**
** Notes:
**
...
void Test_utils(void)
{
    ...
    **   (2)  Call OS_CountSemGive with invalid semID
    */
    ...
    FM_InvokeChildTask();
    TestCount++;
    if ((FM_GlobalData.ChildWriteIndex != 0) || (FM_GlobalData.ChildCmdErrCounter == 0))
    {
        UTF_put_text("FM_InvokeChildTask() -- test failed (1)\n");
        FailCount++;
    }

    /* (2) Roll-over command queue write index */
    FM_GlobalData.ChildCmdErrCounter = 0;
    FM_GlobalData.ChildSemaphore  = 1;
    FM_GlobalData.ChildQueueCount = 0;
    FM_GlobalData.ChildReadIndex  = 0;
    ...
    FM_InvokeChildTask();
    TestCount++;
    if ((FM_GlobalData.ChildWriteIndex != 1) || (FM_GlobalData.ChildCmdErrCounter != 0))
    {
        UTF_put_text("FM_InvokeChildTask() -- test failed (2)\n");
        FailCount++;
    }
