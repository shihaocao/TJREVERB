/*
** $Id: fm_cmd_utils.c 1.31 2011/04/20 11:34:30EDT lwalling Exp  $
**
...
boolean FM_VerifyChildTask(uint32 EventID, char *CmdText)
{
    ...
    else if (FM_GlobalData.ChildQueueCount == FM_CHILD_QUEUE_DEPTH)
    {
        CFE_EVS_SendEvent(EventID, CFE_EVS_ERROR,
             ...
             (FM_GlobalData.ChildWriteIndex >= FM_CHILD_QUEUE_DEPTH))
    {
        CFE_EVS_SendEvent(EventID, CFE_EVS_ERROR,
                         "%s error: child task interface is broken: count = %d, index = %d",
                          CmdText, FM_GlobalData.ChildQueueCount, FM_GlobalData.ChildWriteIndex);
...
void FM_InvokeChildTask(void)
{
    ...
    if (FM_GlobalData.ChildSemaphore != 0)
    {
        ...
        OS_BinSemGive(FM_GlobalData.ChildSemaphore);
    }
