/*************************************************************************
** File:
**   $Id: mm_mem16.c 1.8 2011/12/05 15:17:21GMT-05:00 jmdagost Exp  $
**
** Purpose: 
...
**
**   $Log: mm_mem16.c  $
**   Revision 1.8 2011/12/05 15:17:21GMT-05:00 jmdagost 
                             ...
                             uint32                    DestAddress)
{
       uint32     i          = 0;
       int32      BytesRead  = 0;
       uint16    *AddressPtr = NULL;
       boolean    AtLeastOneRead = FALSE;
       boolean    Valid      = TRUE;
static uint16     DataArray[MM_MAX_LOAD_DATA_SEG/sizeof(uint16)];
   ...
   */
   ...
   {
      AtLeastOneRead = TRUE;
      for (i = 0; i < (BytesRead/sizeof(uint16)); i++)
      {
         CFE_PSP_MemWrite16((uint32)AddressPtr, DataArray[i]);
                           ...
                           MM_LoadDumpFileHeader_t *FileHeader)
{
       ...
       uint16       *DataPtr        = NULL;
static uint16        DataArray[MM_MAX_DUMP_DATA_SEG/sizeof(uint16)];
   ...
   DataPtr = (uint16 *)FileHeader->SymAddress.Offset;
   ...
   if(BytesRemaining <= MM_MAX_DUMP_DATA_SEG)
   {
      ...
      for (i = 0; i < (BytesRemaining/sizeof(uint16)); i++)
      {
          CFE_PSP_MemRead16((uint32)DataPtr, &DataArray[i]);
          DataPtr++;
      }
   ...
   {
      /* Read data blocks and write to file */ 
      while((BytesRemaining / MM_MAX_DUMP_DATA_SEG) >= 1)
      {
         /* Initialize buffer and copy memory  */
         CFE_PSP_MemSet(DataArray, 0, MM_MAX_DUMP_DATA_SEG);

         for (i = 0; i < (MM_MAX_DUMP_DATA_SEG/sizeof(uint16)); i++)
         {
             CFE_PSP_MemRead16((uint32)DataPtr, &DataArray[i]);
             DataPtr++;
         }
...
{
   uint16   i              = 0;
   uint16   FillPattern16  = 0;
   uint32   WritePtr       = 0;
   uint32   BytesRemaining = 0;

   /* 
   ** For the Mem16 type we only use the least significant word of the 
   ** 32 bit fill pattern sent in the command message
   */ 
   FillPattern16 = (uint16)CmdPtr->FillPattern;
   ...
   MM_AppData.MemType      = MM_MEM16;
   MM_AppData.Address      = DestAddress;
   MM_AppData.FillPattern  = FillPattern16;
   ...
   */
   ...
   {
      for (i = 0; i < (MM_MAX_FILL_DATA_SEG/sizeof(uint16)); i++)
      {
         CFE_PSP_MemWrite16(WritePtr, FillPattern16);
