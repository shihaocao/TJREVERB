/*************************************************************************
** File:
**   $Id: mm_dump.c 1.10 2010/11/29 13:35:23GMT-05:00 jmdagost Exp  $
**
** Purpose: 
**   Functions used for processing CFS Memory Manager memory dump commands
**
**   $Log: mm_dump.c  $
**   Revision 1.10 2010/11/29 13:35:23GMT-05:00 jmdagost 
...
**
*************************************************************************/
void MM_FillDumpInEventBuffer(uint32              SrcAddress, 
   ...
   */
   switch(CmdPtr->DataSize)
   {
      case MM_BYTE_BIT_WIDTH:
         CFE_PSP_MemRead8(SrcAddress, &ByteValue);
               ...
               */ 
               if((FileHandle = OS_creat(CmdPtr->FileName, OS_READ_WRITE)) >= 0)
               {
                  ...
                  if(Valid == TRUE)
                  {
                     ...
                     {
                        /* 
                        ** If we get to this point, dump was successful
                        */
                        MM_AppData.CmdCounter++;
...
static uint8      DataArray[MM_MAX_DUMP_DATA_SEG];
   ...
   if(BytesRemaining <= MM_MAX_DUMP_DATA_SEG)
   {
      ...
      CFE_PSP_MemCpy(DataArray, DataPtr, BytesRemaining);
   ...
   else
   {
      ...
      while((BytesRemaining / MM_MAX_DUMP_DATA_SEG) >= 1)
      {
         /* Initialize buffer and copy memory  */
         CFE_PSP_MemSet(DataArray, 0, MM_MAX_DUMP_DATA_SEG);
         CFE_PSP_MemCpy(DataArray, DataPtr, MM_MAX_DUMP_DATA_SEG);
   ...
   if(MM_VerifyCmdLength(MessagePtr, ExpectedLength))
   {
      ...
      if(Valid == TRUE)
      {
         ...
         if(Valid == TRUE)
         {
            ...
            MM_FillDumpInEventBuffer(SrcAddress, CmdPtr, (uint8 *)DumpBuffer);
...
void MM_FillDumpInEventBuffer(uint32              SrcAddress, 
                              ...
                              uint8               *DumpBuffer)
{
#if (MM_OPT_CODE_MEM32_MEMTYPE == TRUE) || (MM_OPT_CODE_MEM16_MEMTYPE == TRUE) || (MM_OPT_CODE_MEM8_MEMTYPE == TRUE)   
   uint32    i;
#endif
   
   /* Initialize buffer */
   CFE_PSP_MemSet(DumpBuffer, 0, MM_MAX_DUMP_INEVENT_BYTES);
   
   switch(CmdPtr->MemType)
   {
      case MM_RAM:
      case MM_EEPROM:
         CFE_PSP_MemCpy((void *)DumpBuffer, (void *)SrcAddress, CmdPtr->NumOfBytes);
         break;
      ...
      case MM_MEM32:
         ...
         {
            CFE_PSP_MemRead32(SrcAddress, (uint32 *)DumpBuffer);
      ...
      case MM_MEM16:
         ...
         {
            CFE_PSP_MemRead16(SrcAddress, (uint16 *)DumpBuffer);
      ...
      case MM_MEM8:
         for (i = 0; i < CmdPtr->NumOfBytes; i++)
         {
            CFE_PSP_MemRead8(SrcAddress, DumpBuffer);
            SrcAddress ++;
            DumpBuffer ++;
         }
         break;
