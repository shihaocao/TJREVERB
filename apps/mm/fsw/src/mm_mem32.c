/*************************************************************************
** File:
**   $Id: mm_mem32.c 1.8 2011/12/05 15:17:21GMT-05:00 jmdagost Exp  $
**
** Purpose: 
                             ...
                             uint32                    DestAddress)
{
       uint32     i           = 0;
       int32      BytesRead   = 0;
       uint32    *AddressPtr  = NULL;
       boolean    AtLeastOneRead = FALSE;
       boolean    Valid       = TRUE;
static uint32     DataArray[MM_MAX_LOAD_DATA_SEG/sizeof(uint32)];
   ...
   */
   ...
   {
      AtLeastOneRead = TRUE;
      for (i = 0; i < (BytesRead/sizeof(uint32)); i++)
      {
         CFE_PSP_MemWrite32((uint32)AddressPtr, DataArray[i]);
...
static uint32     DataArray[MM_MAX_DUMP_DATA_SEG/sizeof(uint32)];
   ...
   DataPtr = (uint32 *)FileHeader->SymAddress.Offset;
   ...
   if(BytesRemaining <= MM_MAX_DUMP_DATA_SEG)
   {
      ...
      for (i = 0; i < (BytesRemaining/sizeof(uint32)); i++)
      {
          CFE_PSP_MemRead32((uint32)DataPtr, &DataArray[i]);
          DataPtr++;
      }
   ...
   else
   {
      ...
      while((BytesRemaining / MM_MAX_DUMP_DATA_SEG) >= 1)
      {
         ...
         for (i = 0; i < (MM_MAX_DUMP_DATA_SEG/sizeof(uint32)); i++)
         {
             CFE_PSP_MemRead32((uint32)DataPtr, &DataArray[i]);
             DataPtr++;
         }
                  ...
                  MM_FillMemCmd_t *CmdPtr)
{
   ...
   */
   while((BytesRemaining / MM_MAX_FILL_DATA_SEG) >= 1)
   {
      ...
      {
         CFE_PSP_MemWrite32(WritePtr, CmdPtr->FillPattern);
