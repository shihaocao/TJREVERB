/*************************************************************************
** File:
**   $Id: cfs_utils.c 1.12 2010/09/21 14:36:34EDT wmoleski Exp  $
**
** Purpose: 
...
{        
    ...
    */ 
    ...
    else
    {
        ...
        */ 
        PacketValue = PacketValue % Algorithm_X;

        if ((Algorithm_O + Algorithm_N) > Algorithm_X)
        {
            /* 
            ** Because of the offset, the not-filtered region wraps...
            */ 
            if (PacketValue >= Algorithm_O)
            {
                /* 
                ** Value is within the not-filtered region before the wrap...
                */ 
                PacketIsFiltered = FALSE;
            }
            else if (PacketValue < ((Algorithm_O + Algorithm_N) - Algorithm_X))
            {
                /* 
                ** Value is within the not-filtered region after the wrap...
                */ 
                PacketIsFiltered = FALSE;
            }
            else
            {
                /* 
                ** Value is not within either of the not-filtered regions...
                */ 
                PacketIsFiltered = TRUE;
            }
        }
        ...
        {
            /* 
            ** The not-filtered region starts and ends without wrapping...
            */ 
            if (PacketValue < Algorithm_O)
            {
                /* 
                ** Value is before the start of the not-filtered region...
                */ 
                PacketIsFiltered = TRUE;
            }
            else if (PacketValue < (Algorithm_O + Algorithm_N))
            {
                /* 
                ** Value is within the not-filtered region...
                */ 
                PacketIsFiltered = FALSE;
            }
            else
            {
                /* 
                ** Value is after the end of the not-filtered region...
                */ 
                PacketIsFiltered = TRUE;
            }
        }
